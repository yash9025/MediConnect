...
const Doctors = () => {
  ...
  return (
    <div className="flex container mx-auto p-4">
      ...
      <div className="w-1/4 bg-gray-200 p-6 rounded-lg shadow-lg mr-6">
        ...
        <ul className="space-y-4">
          {['General physician', 'Gynecologist', 'Dermatologist', 'Pediatricians', 'Neurologist', ''].map((spec) => (
            <li
              key={spec}
              className={`cursor-pointer text-lg ${selectedSpeciality === spec ? 'text-blue-600 font-semibold' : 'text-gray-700'} hover:text-blue-500 transition-all`}
              onClick={() => handleSpecialityClick(spec)}
            >
              {spec}
            </li>
          ))}
          <li className="cursor-pointer text-lg text-gray-700 hover:text-blue-500 transition-all" onClick={handleMoreClick}>
            More
          </li>
        </ul>
      </div>
      ...
      {/* Doctor Cards Area */}
      <div className="w-3/4">
        <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">Browse Doctors by Speciality</h1>

        {speciality && <h2 className="text-2xl font-semibold text-center mb-6 text-gray-600">Speciality: {speciality}</h2>}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredDoctors.length === 0 ? (
            ...
            filteredDoctors.map((doctor) => {
              let readMoreText = selectedDoctor?._id === doctor._id ? 'Read Less' : 'Read More';
              let doctorDetailsClass = selectedDoctor?._id === doctor._id ? 'max-h-screen' : 'max-h-0';

              return (
                <div key={doctor._id} className="doctor-card bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
                  ...
                  <div className="doctor-buttons mb-4">
                    ...
                    <button
                      ...
                      className={`px-6 py-2 rounded-full text-sm transition-all duration-200 cursor-pointer ${
                        doctor.available ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-red-500 text-white cursor-not-allowed'
                      }`}
                    >
                  ...
                  >
                    <div className="bg-gray-50 p-4 rounded-lg mt-4">
                      <h4 className="text-lg font-semibold text-gray-800 mb-2">About {doctor.name}</h4>
                      <p className="text-sm text-gray-600 mb-4">{doctor.about}</p>
                      <p className="text-sm text-gray-600 mb-2">Fees: ${doctor.fees}</p>
                      <p className="text-sm text-gray-600">Location: {doctor.address.line1}, {doctor.address.line2}</p>
                    </div>
                  </div>
                </div>
              );
